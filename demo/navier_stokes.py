...
if mpi_comm.size == 1:
    ...
    #
    # We write in the same format as would be generated by `meshutil.import_gmsh`,
    # so that we can read either kind of mesh with the same code below.
    #
    # Note, however, that our `domain_parts` are specified w.r.t. `mesh`, not `fluid_mesh`,
    # so they are not applicable here. But we don't need subdomain data, so we can just leave it out.
