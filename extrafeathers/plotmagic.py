...
def as_mpl_triangulation(V: dolfin.FunctionSpace, *,
    ...
    n = 3 if cell_kind == "triangle" else 4
    polys = mcoll.PolyCollection([[our_nodes[dof] for dof in cell[:n]] for cell in cells])
...
def mpiplot_mesh(V: dolfin.FunctionSpace, *,
    ...
    if show_partitioning:  # color-code MPI partitions
        ignored_polys, my_triangulation = as_mpl_triangulation(V, mpi_global=False, refine=True)
        ...
        if V.ufl_element().degree() > 1:
            ignored_polys, my_triangulation = as_mpl_triangulation(V, mpi_global=False, refine=False)
            all_triangulations = dolfin.MPI.comm_world.gather(my_triangulation, root=0)
            if dolfin.MPI.comm_world.rank == 0:
                for k, tri in enumerate(all_triangulations):
                    if not tri:
    ...
    if not _triangulation:  # TODO: doesn't interact with `show_partitioning`
        ignored_polys, _triangulation = as_mpl_triangulation(V, refine=True)
    main_plot = None
    if show_aux or V.ufl_element().degree() <= 1:
        if dolfin.MPI.comm_world.rank == 0:
    ...
    if V.ufl_element().degree() > 1:
        ignored_polys, element_edges = as_mpl_triangulation(V, refine=False)
        if dolfin.MPI.comm_world.rank == 0:
