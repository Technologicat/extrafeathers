...
def as_mpl_triangulation(V: dolfin.FunctionSpace, *,
    ...
    `mpi_global`: If `True`, represent the whole mesh.
                  ...
                  has no cells, the `polys` and `tris` attributes of the return value
                  are both `None`.
...
def mpiplot_mesh(V: dolfin.FunctionSpace, *,
    ...
    Also, is able to show the degree-1 vis edges (generated by `extrafeathers`)
    if `V` is degree-2 or degree-3.
    ...
    `show_aux`: Whether to plot the vis edges if present.
    ...
    if V.ufl_element().degree() > 1:
        if show_partitioning:
            ...
            prep = as_mpl_triangulation(V, mpi_global=True, refine=False)
            all_polys = [prep.polys]
        if dolfin.MPI.comm_world.rank == 0:
            for mpi_rank, polys in enumerate(all_polys):
                if not polys:
    ...
    if show_partitioning and dolfin.MPI.comm_world.rank == 0:
