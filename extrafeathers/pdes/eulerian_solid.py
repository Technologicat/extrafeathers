...
class EulerianSolid:
    ...
    def compile_forms(self) -> None:
        ...
        if self.τ == 0.0:  # Linear elastic (LE)
            ...
            F_σ = (inner(Σ, φ) * dx -
                   (inner(K_inner_εu + τ * K_inner_εv, sym(φ)) * dx +  # linear elastic and τ ∂/∂t (...)
...
class SteadyStateEulerianSolid:
    ...
    def compile_forms(self) -> None:
        ...
        # Choose constitutive equation
        if self.τ == 0.0:  # Linear elastic (LE)
            F_σ = (inner(σ, φ) * dx - inner(K_inner_εu, φ) * dx)
        else:  # Axially moving Kelvin-Voigt (KV)
        ...
        # TODO: This only actually has an `u` when `a ≠ 0`. Maybe need a monolithic formulation?
